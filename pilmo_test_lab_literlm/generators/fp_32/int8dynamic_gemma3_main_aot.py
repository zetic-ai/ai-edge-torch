...
def main():
    output_dir = "/home/pilmo/workspace/ai-edge-torch/pilmo_test_lab_literlm/bin/int8_rollback_main"
    os.makedirs(output_dir, exist_ok=True)
    tflite_path = os.path.join(output_dir, "gemma3_1b_main.tflite")
    ...
    sample_kwargs = {
        "embeddings": torch.zeros((1, 1, EMBED_DIM), dtype=torch.int32),
        "mask_global": torch.zeros((1, 1, 1, MASK_LEN), dtype=torch.int32),
        "mask_local": torch.zeros((1, 1, 1, MASK_LEN), dtype=torch.int32),
        "pos_emb_cos": torch.zeros((1, 1, 1, HEAD_DIM), dtype=torch.int32),
        "pos_emb_sin": torch.zeros((1, 1, 1, HEAD_DIM), dtype=torch.int32),
        "pos_emb_local_cos": torch.zeros((1, 1, 1, HEAD_DIM), dtype=torch.int32),
        "pos_emb_local_sin": torch.zeros((1, 1, 1, HEAD_DIM), dtype=torch.int32),
    }
    for i in range(NUM_LAYERS):
        sample_kwargs[f"kv_cache_k_{i}"] = torch.zeros(
            (1, 1, KV_CACHE_LEN, HEAD_DIM), dtype=torch.int32
        )
        sample_kwargs[f"kv_cache_v_{i}"] = torch.zeros(
            (1, 1, HEAD_DIM, KV_CACHE_LEN), dtype=torch.int32
        )

    print(
        f"Exporting Gemma3-1B (26 layers) No-Pos Model (Cache 1280, Mask 1281/1408)..."
    )
    ...
    prefill_kwargs = {
        "embeddings": torch.zeros((1, PREFILL_T, EMBED_DIM), dtype=torch.int32),
        "mask_global": torch.zeros(
            (1, 1, PREFILL_T, PREFILL_MASK_LEN), dtype=torch.int32
        ),
        "mask_local": torch.zeros(
            (1, 1, PREFILL_T, PREFILL_MASK_LEN), dtype=torch.int32
        ),
        "pos_emb_cos": torch.zeros((1, PREFILL_T, 1, HEAD_DIM), dtype=torch.int32),
        "pos_emb_sin": torch.zeros((1, PREFILL_T, 1, HEAD_DIM), dtype=torch.int32),
        "pos_emb_local_cos": torch.zeros(
            (1, PREFILL_T, 1, HEAD_DIM), dtype=torch.int32
        ),
    ...
    # NEW: Apply industrial-grade graph fusion (mu_pass_lib)
    print("Applying industrial-grade graph fusion (mu_pass_lib)...")
    from ai_edge_torch.generative.export_hf.core.mu import mu_pass_lib

    optimized_tflite_path = tflite_path.replace(".tflite", "_optimized.tflite")
    mu_pass_lib.update_model(tflite_path, optimized_tflite_path)

    print("AOT Compiling (Fusing to single DISPATCH_OP)...")
    try:
        # Use the optimized TFLite for AOT compilation
        with open(optimized_tflite_path, "rb") as f:
            tflite_bytes = f.read()
        ...
        if result.models_with_backend:
            ...
            print(f"SUCCESS: Gemma3-1B Main No-Pos Model saved at {aot_tflite_path}")
