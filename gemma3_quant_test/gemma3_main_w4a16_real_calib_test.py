...
def main():
    ...
    tflite_path = os.path.join(output_dir, "gemma3_1b_main_fp32_for_real_w4a16.tflite")
    w4a16_real_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_real_calib.tflite")
    aot_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_real_calib_aot.tflite")
    ...
    # Matching the 58 inputs EXACTLY
    # [IMPORTANT] Using torch.randn instead of torch.zeros to provide real scale ranges
    torch.manual_seed(42)
    sample_kwargs = {
        "embeddings": torch.randn((1, 1, EMBED_DIM)),
        "mask_global": torch.zeros((1, 1, 1, MASK_LEN)),  # Mask can stay zero
        "mask_local": torch.zeros((1, 1, 1, MASK_LEN)),
        "pos_emb_cos": torch.randn((1, 1, 1, HEAD_DIM)),
        "pos_emb_sin": torch.randn((1, 1, 1, HEAD_DIM)),
        "pos_emb_local_cos": torch.randn((1, 1, 1, HEAD_DIM)),
        "pos_emb_local_sin": torch.randn((1, 1, 1, HEAD_DIM)),
    }
    for i in range(NUM_LAYERS):
        sample_kwargs[f"kv_cache_k_{i}"] = torch.randn((1, 1, KV_CACHE_LEN, HEAD_DIM))
        sample_kwargs[f"kv_cache_v_{i}"] = torch.randn((1, 1, HEAD_DIM, KV_CACHE_LEN))
    ...
    # 2. Manual W4A16 Quantization Setup (Symmetric)
    print("[2/4] Setting up Manual W4A16 Symmetric Quantization...")
    ...
    # 3. Calibrate with Real Random Data and Quantize
    print("[3/4] Calibrating with Random Data and Quantizing to W4A16...")
    calib_data = {"decode": [sample_kwargs]}
    res = qt.calibrate(calib_data)
    quant_result = qt.quantize(res)
    quant_result.export_model(w4a16_real_path, overwrite=True)
    ...
    try:
        ...
        if aot_res.models_with_backend:
            ...
            print(f"✅ Success! Realistic W4A16 AOT model created: {aot_path}")
        else:
