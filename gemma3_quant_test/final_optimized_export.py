...
# MONKEYPATCH for Weight Sharing
original_apply_plugin = qnn_backend._apply_plugin


@qnn_backend._call_component.register
def patched_apply_plugin(
    ...
    # This matches the logic in qualcomm_backend.py but adds the missing flag
    import os, pathlib
    ...
    extra_kwargs = {
        ...
        "qualcomm_enable_weight_sharing": "true",  # FORCE ENABLE
    }
    return component(
        input_model,
        output_model,
...
class Gemma3Main(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # USER REQUEST: Prefill (T > 1) does NOT return logits
...
def main():
    ...
    print("\n--- [1/2] MAIN MODEL (W4A16, Unified Signatures) ---")
    tflite_path = os.path.join(output_dir, "optimized_main_v2_fp32.tflite")
    w4a16_path = os.path.join(output_dir, "optimized_main_v2_w4a16.tflite")
    aot_path = os.path.join(output_dir, "optimized_main_v2_w4a16_aot.tflite")
    ...
    print("AOT Compiling Main (Dual Signatures)...")
    ...
    # Discovered flag from apply_plugin_main --helpfull
    aot_config = [
        litert_types.CompilationConfig(
            target=target, qualcomm_enable_weight_sharing=True
        )
    ]
    ...
    print("\n--- [2/2] EMBEDDER (W4) ---")
    embed_fp32 = os.path.join(output_dir, "optimized_embedder_v2_fp32.tflite")
    embed_w4 = os.path.join(output_dir, "optimized_embedder_v2_w4.tflite")

    # Custom Embedder module to handle 'input' vs 'token_ids' correctly
    class EmbedWrapper(nn.Module):
        ...
        def forward(
            self, input
        ):  # Named 'input' to match nn.Embedding typical use or just generic
            return self.emb(input)

    embed_mod = EmbedWrapper().eval()
    ...
    final_path = os.path.join(output_dir, "gemma3_1b_w4a16_v2_final.litertlm")
