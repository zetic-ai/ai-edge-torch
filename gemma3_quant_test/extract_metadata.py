...
def extract_metadata(src_path, dst_path):
    with open(src_path, "rb") as f:
        ...
        for i in range(sec_metadata.ObjectsLength()):
            ...
            if obj.DataType() == schema.AnySectionDataType.LlmMetadataProto:
                begin = obj.BeginOffset()
                end = obj.EndOffset()
                size = end - begin
                print(f"Found LLMMetaData at {begin}-{end}, size {size}")
                f.seek(begin)
                data = f.read(size)
...
if __name__ == "__main__":
    ...
    dst = "gemma3_quant_test/output/llm_metadata.pb"
    extract_metadata(src, dst)
