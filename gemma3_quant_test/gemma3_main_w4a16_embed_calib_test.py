...
def generate_calibration_samples(num_samples=5, to_numpy=True):
    ...
    with torch.no_grad():
        for _ in range(num_samples):
            ...
            tokens = torch.randint(0, VOCAB_SIZE, (1, 1), dtype=torch.int32)
...
def main():
    ...
    tflite_path = os.path.join(output_dir, "gemma3_1b_main_fp32_for_embed_calib.tflite")
    w4a16_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_embed_calib.tflite")
    aot_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_embed_calib_aot.tflite")
    ...
    sample_kwargs = generate_calibration_samples(1, to_numpy=False)[0]
    ...
    # 2. Setup W4A16 Manual Symmetric
    ...
    # 3. Calibrate with Embedder Outputs
    print("[3/4] Calibrating with Embedder-generated data...")
    calib_samples = generate_calibration_samples(num_samples=10)
    calib_data = {"decode": calib_samples}
    ...
    try:
        ...
        if aot_res.models_with_backend:
            ...
            print(f"✅ Success! Embed-Calibrated W4A16 AOT model created: {aot_path}")
        else:
