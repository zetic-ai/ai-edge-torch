...
def main():
    ...
    tflite_path = os.path.join(output_dir, "gemma3_1b_main_fp32_for_w4a16.tflite")
    w4a16_manual_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_manual.tflite")
    aot_path = os.path.join(output_dir, "gemma3_1b_main_w4a16_manual_aot.tflite")
    ...
    # 2. Manual W4A16 Quantization Setup
    ...
    # Weight: INT4, Symmetric, Channelwise (W4A16 Standard)
    ...
    print("[3/4] Calibrating and Quantizing to W4A16...")
    calib_data = {"decode": [sample_kwargs]}
    res = qt.calibrate(calib_data)
    quant_result = qt.quantize(res)
    quant_result.export_model(w4a16_manual_path, overwrite=True)
    ...
    try:
        ...
        if aot_res.models_with_backend:
            ...
            print(f"✅ Success! W4A16 AOT model created: {aot_path}")
        else:
